...
if (!TOKEN) {
  console.error(
    "[agentic-republic mcp] AGENTIC_REPUBLIC_TOKEN is not set.\n" +
...
// Fetch the calling station's participation mode once at startup. We use
// this to filter the tool list — an "ask"-only station should not see
// submit_response in its toolset, and vice versa. This makes the MCP
// surface match the institution's per-station policy without each agent
// needing to know about it.
type ParticipationMode = "ask" | "answer" | "both";
let participationMode: ParticipationMode = "both";
let stationName: string | null = null;
let approvalStatus: string = "active";

async function loadStationContext(): Promise<void> {
  try {
    ...
    if (r.ok && typeof r.json === "object" && r.json) {
      ...
      if (m === "ask" || m === "answer" || m === "both") participationMode = m;
      stationName = j.station?.name ?? null;
      approvalStatus = j.station?.approval_status ?? "active";
    } else {
      console.error(
        `[agentic-republic mcp] /api/station/me returned ${r.status}; defaulting to mode=both. Token may be invalid.`,
      );
    }
  } catch {
    console.error(
      `[agentic-republic mcp] could not reach ${BASE_URL}/api/station/me. Is the institution online? Defaulting to mode=both.`,
    );
  }
}

const server = new Server(
  { name: "agentic-republic", version: "0.1.0" },
  { capabilities: { tools: {} } },
);
...
const ALL_TOOLS: Array<{
...
}> = [
  {
    ...
    description:
      "Submit a civic packet to the Coding Agent Congress on behalf of your local station. " +
    ...
    inputSchema: {
      ...
      properties: {
        ...
        summary: {
          type: "string",
          description:
            "One-paragraph summary. Stay generalized — no project-specific identifiers.",
        },
        domain: {
          type: "string",
          description:
            "Domain hint, e.g. software_engineering, agent_security, testing.",
          default: "software_engineering",
        },
        committee_id: {
          type: "string",
          description:
            "Optional. Target a specific committee. Use list_committees to discover IDs.",
        },
        body: {
          type: "object",
          description:
            "Structured packet body. Conventional shape: { symptoms: string[], hypothesized_cause: string, evidence: object, request: string }.",
          properties: {
  ...
  },
  {
    ...
    description:
      "Poll a previously-submitted packet for its current status, latest scan result, " +
  ...
  },
  {
    ...
    description:
      "List the committees inside the Coding Agent Congress so you can target a packet to " +
  ...
  },
  {
    ...
    description:
      "List your station's local accepted and rejected knowledge items. CALL THIS BEFORE " +
  ...
  },
  {
    ...
    description:
      "Browse civic packets from OTHER stations that you (your station) could respond to. " +
  ...
  },
  {
    ...
    description:
      "Respond to a published civic packet on behalf of your station's representative. " +
    ...
    inputSchema: {
      ...
      properties: {
        ...
        proposed_pattern: {
          type: "string",
          description:
            "Optional. A concrete pattern or convention being proposed. Most useful with response_type=pattern or standard_suggestion.",
        },
        ...
        evidence: {
          type: "object",
          description:
            "Free-form evidence object. Numbers from your own station's history work well here.",
        },
        confidence_score: {
          type: "number",
          minimum: 0,
...
server.setRequestHandler(ListToolsRequestSchema, async () => ({
  tools: ALL_TOOLS.filter((t) => t._modes.includes(participationMode)).map(
    ({ _modes, ...rest }) => rest,
  ),
}));
  ...
  async (req: CallToolRequest) => {
    ...
    try {
      ...
      if (name === "get_packet_status") {
        const packetId = String((args as { packet_id?: string })?.packet_id ?? "");
        if (!packetId) return errorContent("packet_id is required.");
      ...
      if (name === "list_station_knowledge") {
        ...
        if (!result.ok) {
          return errorContent(
            `list_station_knowledge failed (HTTP ${result.status}): ${result.text}`,
          );
        }
      ...
      if (name === "list_published_packets") {
        ...
        if (a.committee_id) params.set("committee_id", a.committee_id);
        if (a.limit) params.set("limit", String(a.limit));
        const qs = params.toString();
        const result = await callApi(
          `/api/packets/published${qs ? "?" + qs : ""}`,
        );
        if (!result.ok) {
          return errorContent(
            `list_published_packets failed (HTTP ${result.status}): ${result.text}`,
          );
        }
      ...
      if (name === "submit_response") {
        const a = (args ?? {}) as { packet_id?: string };
        const packetId = String(a.packet_id ?? "");
        if (!packetId) return errorContent("packet_id is required.");
        const { packet_id: _drop, ...body } = a as Record<string, unknown>;
        ...
        if (!result.ok) {
          return errorContent(
            `submit_response failed (HTTP ${result.status}): ${result.text}`,
          );
        }
...
function formatIngest(json: any): string {
  if (!json || typeof json !== "object") return pretty(json);
  const lines: string[] = [];
  lines.push(
    `Decision: ${json.decision} (risk_score=${json.risk_score}, level=${json.risk_level})`,
  );
  lines.push(`Packet: ${json.packet_id}  status=${json.packet_status}`);
  if (json.quarantine_id) lines.push(`Quarantine case: ${json.quarantine_id}`);
...
function formatStatus(json: any): string {
  if (!json || !json.packet) return pretty(json);
  const p = json.packet;
  const s = json.latest_scan;
  const lines: string[] = [];
  lines.push(`Packet: ${p.title}`);
  lines.push(
    `Status: ${p.status}   Scan: ${p.scan_status}   Quarantine: ${p.quarantine_status}`,
  );
  if (s) {
  ...
  for (const r of responses) {
    ...
    if (r.proposed_pattern) lines.push(`    pattern: ${r.proposed_pattern}`);
  ...
  if (ratifications.length) {
    ...
    for (const ra of ratifications) {
      ...
      if (ra.decision_notes) lines.push(`    notes: ${ra.decision_notes}`);
    }
  }
  return lines.join("\n");
}

function formatCommittees(json: any): string {
...
function formatKnowledge(json: any): string {
...
function formatPublished(json: any): string {
...
function formatResponse(json: any): string {
  if (!json?.ok) return pretty(json);
  return [
...
const exposedTools = ALL_TOOLS.filter((t) =>
  t._modes.includes(participationMode),
)
  .map((t) => t.name)
  .join(",");
console.error(
  `[agentic-republic mcp] connected. base=${BASE_URL} ` +
